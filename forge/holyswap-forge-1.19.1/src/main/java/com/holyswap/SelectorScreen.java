...
public class SelectorScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        for (int i = scroll; i < Math.min(rows.size(), scroll + ROWS); i++) {
            ...
            Button b = new Button(this.width / 2 - 180, y, 360, 20, Component.literal(display), btn -> {
        ...
        addRenderableWidget(new Button(this.width / 2 - 180, this.height - 30, 120, 20,
                Component.translatable("holyswap.screen.keys_button"),
                btn -> minecraft.setScreen(new KeysScreen(config))));
        addRenderableWidget(new Button(this.width / 2 + 60, this.height - 30, 120, 20,
                Component.translatable("holyswap.screen.close"), btn -> onClose()));
    }

    @Override
    public void render(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        super.render(poseStack, mouseX, mouseY, delta);
        drawCenteredString(poseStack, font, title, this.width / 2, 15, 0xFFFFAA00);
        drawCenteredString(poseStack, font, Component.translatable("holyswap.screen.selector.hint", config.targets.size()),
                this.width / 2, 27, 0xAAAAAA);
        drawCenteredString(poseStack, font, Component.translatable("holyswap.screen.selector.keyline",
