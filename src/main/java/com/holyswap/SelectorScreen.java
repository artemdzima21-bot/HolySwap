...
public class SelectorScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        Text keys = Text.translatable("holyswap.screen.selector.keyline",
                keyOf(SwapConfig.ACT_SPHERE), keyOf(SwapConfig.ACT_SPHERE_PLUS),
                keyOf(SwapConfig.ACT_TALISMAN), keyOf(SwapConfig.ACT_TALISMAN_PLUS),
                keyOf(SwapConfig.ACT_TOTEM));
        ctx.drawCenteredTextWithShadow(textRenderer, hint, this.width / 2, 27, 0xAAAAAA);
        ctx.drawCenteredTextWithShadow(textRenderer, keys, this.width / 2, 37, 0xAAAAAA);
    }
