...
public class SelectorScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        addButton(new ButtonWidget(this.width / 2 - 180, this.height - 30, 120, 20,
                new TranslatableText("holyswap.screen.keys_button"),
                btn -> client.openScreen(new KeysScreen(config))));
        addButton(new ButtonWidget(this.width / 2 + 60, this.height - 30, 120, 20,
                new TranslatableText("holyswap.screen.close"), btn -> close()));
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        String hint = new TranslatableText("holyswap.screen.selector.hint", config.targets.size()).getString();
        String keys = new TranslatableText("holyswap.screen.selector.keyline",
                keyOf(SwapConfig.ACT_TALISMAN), keyOf(SwapConfig.ACT_SPHERE),
                keyOf(SwapConfig.ACT_TALISMAN_PLUS), keyOf(SwapConfig.ACT_TOTEM)).getString();
        drawCenteredText(matrices, textRenderer, hint, this.width / 2, 27, 0xAAAAAA);
        drawCenteredText(matrices, textRenderer, keys, this.width / 2, 37, 0xAAAAAA);
    }
    ...
    private String keyOf(String action) {
        ...
        String s = net.minecraft.client.util.InputUtil.fromKeyCode(code, 0).getLocalizedText().getString();
